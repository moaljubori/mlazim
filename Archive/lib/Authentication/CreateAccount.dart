...
// -----  Email Input   ^^^^^^^^^^^^^^^^^^^^^^^^^^
          Container(
            color: isLightTheme.value? Colors.white : const Color.fromRGBO(1, 38, 63, 1),
            child: Form(
...
if (_email.text == '' &&_phone.text == '' &&_password.text == '' && _Repassword.text == '') {
showDialog<void>(
context: context,
barrierDismissible: false,
builder: (BuildContext context) {
return AlertDialog(
title: Text('Please_enter'.tr ,  textAlign: TextAlign.center,),
content: SingleChildScrollView(
child: ListBody(
children: <Widget>[Text('Please_try_again!!'.tr , textAlign: TextAlign.center,),],
),
),
actions: <Widget>[
TextButton(child: Text('Returnx'.tr),onPressed: () {Navigator.of(context).pop();},
),],
);
},
);
...
'ServerApi/CreateUser.php?email=' +_email.text +'&password=' +_password.text +'&phone=' +sendnumber);
Map? responseBody = response.data;
for (var o in responseBody!['state']) {
if (o['u'] == '1') {
//Navigator.pushNamed(context, '/register_pageEnd',arguments: {'id': o['id']});
Get.to( register_pageEnd(id: o['id'],));
}
...
return AlertDialog(
                                  title:
                                      Text('The_account_already_exists!!'.tr ,  textAlign: TextAlign.center,),
                                  content: SingleChildScrollView(
                                    child: ListBody(
                                      children: <Widget>[
                                        Text(
...
return AlertDialog(
                                  title:
                                      Text('The_Number_Phone_already_exists!!'.tr ,  textAlign: TextAlign.center,),
                                  content: SingleChildScrollView(
                                    child: ListBody(
                                      children: <Widget>[
                                        Text(
...
showDialog<void>(
                              ...
                              builder: (BuildContext context) {
                                return AlertDialog(
                                  ...
                                  content: SingleChildScrollView(
                                    child: ListBody(
                                      children: <Widget>[
                                        Text('Unknown problem in adding account'
                                            .tr , textAlign: TextAlign.center,),
                                      ],
                                    ),
                                  ),
                      ...
                      showDialog<void>(
                        ...
                        builder: (BuildContext context) {
                          return AlertDialog(
                            title: Text('Passwords do not match'.tr ,  textAlign: TextAlign.center,),
                            content: SingleChildScrollView(
                              child: ListBody(
                                children: <Widget>[
                                  Text('Please try again!!'.tr , textAlign: TextAlign.center,),
                                ],
                              ),
                            ),
                            actions: <Widget>[
